...
const agora_rtm_init = async (appId, userId) => {
  if (agoraRTMInstance === null) {
    agoraRTMInstance = AgoraRTM.createInstance(appId, {enableLogUpload: false});
...
function Homepage() {
  useEffect(() => {
    agora_rtm_init(localStorage.getItem("agora_app_id"), localStorage.getItem("userid"))
      .then(() => {console.log("Agora RTM instance created and logged in")});
  }, [])
